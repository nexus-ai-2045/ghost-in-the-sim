...
初期画面は研究用dashboardではなく、御影冴として鏡潮事案へ介入する作戦端末とする。日本語で事件状況と御影・真壁の役割を読み、3つの介入方針から一つを選び、作戦開始後は生成済みtrajectoryを12ターンに分けて前後移動・再開できる。最終ターンでは作戦完了の要約（最終状態、真壁の停止要求があったターン、現れた代償）を示し、別の方針で再挑戦できる。方針名は「中央正本型」のような研究用語を前面に出さず、内部condition IDとの対応をJavaScript内の明示的mappingに限定した日本語で表示する。seed、run_id、schema名、replay evidence、比較指標は折り畳んだ監査／反実仮想ビューへ置き、内部識別子をプレイヤーへ露出しない。
...
| 領域 | 表示内容 | 主な操作 |
|---|---|---|
| 作戦端末 | 事件状況、御影・真壁の役割、3介入方針（日本語）、注意配分の説明、真壁の停止要求、成功見込み、代償、作戦完了要約 | 方針選択、作戦開始、前後ターン、最初から再開、別の方針で再挑戦 |
